...
/* The five-step creation journey.
 *
 * It is a client component so the steps can be walked without a round trip, but it decides
 * nothing about who may use it. The server page above it has already required a Clerk session
 * and already established that this member has no profile yet; this form only collects. */
export function OnboardingForm() {
  ...
  /* Both of these are new, and they are the whole point of this pass: every fetch below had
     exactly one branch — the happy one — so a 401, a 409 or a 500 left the button looking
     idle and the villager looking at nothing. A failure the person cannot see is worse than
     an error message; it reads as a broken site. */
  ...
  /* A full navigation rather than a router push: the session is gone, so the server has to
     re-evaluate who this is. `redirect_url` brings them back to this page afterwards. */
  function toSignIn() {
    ...
    window.location.href = '/sign-in?redirect_url=%2Findigenes%2Fonboarding'
  }
  ...
  async function uploadPhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (!res.ok || !data.url) {
        /* The upload route already writes messages a villager can act on — the size limit,
           the accepted formats — so its own words are shown rather than replaced. */
  ...
  async function submit() {
    ...
    try {
      ...
      if (res.ok) {
        /* Straight to the profile they now own, rather than to the directory. The first
           thing a person wants after filling in a form about themselves is to see what it
           made — and that page is also where they will go to change it. */
        router.push('/indigenes/profile?created=1')
        return
      }

      /* The session expired mid-form. Send them to sign in and bring them back here. */
      if (res.status === 401) return toSignIn()

      /* They already have a profile — two tabs, a double submit, or a create that raced
         another. Not an error to argue with: take them to the profile that exists. */
      if (res.status === 409) {
        router.replace(typeof data.profileUrl === 'string' ? data.profileUrl : '/indigenes/profile')
        return
      }
  ...
  return (
    <main style={{ backgroundColor:'oklch(0.965 0.012 85)', minHeight:'100vh',
      ...
      <div style={{ maxWidth:'680px', margin:'0 auto', padding:'0 1.5rem' }}>
        ...
        {step === 0 && (
          <div style={{ display:'flex', flexDirection:'column', gap:'1.5rem' }}>
            <div style={{ textAlign:'center' }}>
              ...
              {photoError && (
                <p role="alert" style={{ color:'oklch(0.400 0.120 25)', fontFamily:'Inter, sans-serif', fontSize:'0.8rem', marginTop:'0.5rem' }}>
                  {photoError}
                </p>
              )}
            </div>
        ...
        {error && (
          <p role="alert" style={{ backgroundColor:'oklch(0.400 0.120 25 / 0.07)', border:'1px solid oklch(0.400 0.120 25 / 0.35)', color:'oklch(0.400 0.120 25)', fontFamily:'Inter, sans-serif', fontSize:'0.88rem', lineHeight:1.6, padding:'0.85rem 1rem', marginTop:'2rem', marginBottom:0 }}>
            {error}
          </p>
        )}
