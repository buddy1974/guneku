...
## 2026-09-16 - The site kept a member register and a Palace inbox, and published no privacy policy

Google rejected the Fondom's OAuth branding verification for two reasons. One is domain
ownership, which is handled outside this repository. The other is ours: the privacy policy URL
had been configured as `https://www.guneku.org/palace`, and the Palace's own page is not a
privacy policy. The URL promised something no page delivered.

**Added `/privacy` and `/terms`**, written from the implementation rather than from a
generator.

The legacy Joomla site did carry both documents — `src/data/pages/privacy-policy.json` and
`src/data/pages/terms.json`, generated by TermsFeed in May 2023. They call Guneku "the
Company", define *Affiliate* in terms of share ownership, speak of "acceptance and
consideration of payment", and name the governing law as that "of cm" — a generator's unfilled
country-code placeholder. They predate Clerk, the database, the indigenes register, the
assistant and the human check, so every concrete claim in them is wrong or unverifiable. No
route renders them; they stay where they are as migrated evidence and nothing in the new pages
comes from them.

**What the new pages say was read off the code**: `middleware.ts` for what requires an account,
the five migration files for what is actually stored, the four public form routes for what a
visitor is asked to give, `src/lib/rate-limit.ts` for the network address, `src/lib/
turnstile.ts` for the human check, `src/app/api/indigenes/upload/route.ts` for images, and
`package.json` for the processors — Vercel, Clerk, Neon, Resend, Cloudflare, Anthropic, and
Google only where somebody chooses to sign in with it.

**Two sentences that were tempting to write more comfortably.** An uploaded photograph is
served from a public URL — `@vercel/blob` is called with `access: 'public'` — so the policy
says the link is unguessable but not private, rather than "your photo is private". And there is
no automated account-deletion button on this site, so the policy says removal is done by asking
the Palace, rather than promising a mechanism that does not exist. No retention periods were
invented: the policy states the principle for each category and then says plainly that fixed
deletion periods have not been set.

**What the terms deliberately omit**: payment and subscription terms, warranties, an
arbitration clause, a named court, a governing jurisdiction and a corporate entity. Guneku
charges nobody, is not a company, and has settled none of those. When it settles one, it
belongs there — written from the decision.

**Navigation.** Both sit in a labelled `Legal` nav in the footer's bottom bar, reachable from
every page, and deliberately not in Explore: they are reference, not the record a villager
comes to read. Both are in the sitemap at low priority, and the robots policy is unchanged.

Twenty-five tests: both routes exist and carry self-referencing canonicals; each page addresses
the subjects a policy of its kind has to address, behind a length floor that fails if either is
ever replaced by a stub; the footer links both and Explore does not; the sitemap gains them
without relaxing what it holds back; `/palace` contains no privacy policy and `next.config.ts`
routes no legal path through it; and every site-root-relative link the footer renders still
...
## 2026-09-17 - A product pass, and a programme that was never ours

Run brief: close Track A, recover and build Track B, finish the remaining roadmap, accept the
product, and stop short of the SEO pass.

### Track A and Track B are not Guneku's

Established by looking. None of the three anchor commits named in the brief — `8168845`,
`fcf6d78`, `bfc472f` — exists here; all 120 commits were searched. No commit message in the
history mentions either track. None of the named capabilities — Capability Discovery, Idea
Inbox, Learn OpenClaw, Tool Intelligence, Opportunity Discovery, Morning Brief, Personal
Context — exists as a feature in the tree. "OpenClaw" appears twice: `.openclaw/project.yaml`,
a one-way export generated by OpenClaw Workspace Intelligence, and a handover report naming
OpenClaw as the agent inheriting this project. It is the tool that scans the portfolio;
Guneku is a thing it scans.

**Nothing was invented to fill the gap.** The same brief forbids inventing historical
requirements, and a fabricated Track B would have been a large body of work the Fondom never
asked for. Guneku's actual programme is Phases 0-15 in `programme-architecture.md`, complete
but for items that each need an owner fact or an owner action.

So this became what the repository actually supported: a product pass over the finished
thing.

### Dead ends and detours, found by crawling production

293 pages, 293 unique internal link targets. One link was dead and three worked only because
a redirect caught them.

`/quarters/ngong` pointed at `/institutions/guneku-agro-cig`, dead twice over — the CIG
has its own home at `/agro-cig` and its id is `agro-cig`. An invariant already guarded this
exact mistake, but only inside `explore/locations.json`, the file the first two instances were
found in. Guarding one file against something that can happen in any of them is how it reached
production a third time; the sweep now covers every file under `src/data`.

The header and the home page both pointed at `/gallery/videos`, and Palace AI cited
`/notables/<slug>`. Both have redirected since those sections moved. A redirect is for links
written down elsewhere, not for our own navigation: these cost a round trip each on a site
whose readers are largely on a throttled connection, and Palace AI was handing out a stale
address as a citation. A second invariant now reads the redirect table out of
`next.config.ts` and fails if anything under `src/` links at one of its sources.

Both guards were checked by reintroducing the defect and watching them name the file.

### The human check was scrolling the whole page sideways

Measured in a real 320-414px viewport. Cloudflare renders Turnstile at a fixed 300px, and a
fixed width in a normal-flow block propagates outwards as a floor every ancestor honours. On
`/contact` it made the single mobile column 366px wide inside 333px of room:

| width | page scrolled sideways |
|---|---|
| 320px | 79px |
| 360px | 39px |
| 390px | 9px |
...
`/support` and `/indigenes/submit` did the same, 55px at 320px.

`contain: inline-size` is what settled it, and it had to be containment rather than overflow.
`overflow-x: auto` alone changed nothing — measured, 79px before and after — because the
automatic minimum size of zero that scroll containers get applies to flex and grid items, not
to a block in normal flow. Padding tuning was measured too, and fixes one width at a time:
32px to 16px clears 390px and leaves 47px at 320px.

One change, in the one component that renders the widget, so all four forms are covered.
Verified on production afterwards: zero page overflow at 320, 360 and 390. R-068 records what
remains — the widget itself scrolls a little inside its own box on a narrow phone, and
Cloudflare's compact size is an owner decision.

### A badge that would not give way

The Fon's patronage list pushed the page 6px past the edge at 320px: the role badge carried
`flex-shrink: 0`, so "Patron and Founder-sponsor" held its full 191px in a 221px row.

The first attempt was wrong and is recorded because it was measured and discarded rather than
shipped: `flex-wrap: wrap` dropped the badge below the name at **every** width, the name's
max-content being wider than the 295px row. That traded a 6px overflow for a layout change on
every screen. Letting the badge shrink instead is identical at 390px, 768px and on the
desktop, and at 320px it takes a second line inside its own badge and stays where the design
puts it.

### Typing the Fon's record instead of casting around it

That page failed ESLint with ten pre-existing `no-explicit-any` errors, all one shape:
`FonProfile` declares an index signature, so any field not named on the interface arrives as
`unknown` and the page cast itself to `any` to render it.

Two dead reads were hiding behind those casts, which is the argument for naming fields.
`enthronementDateDisplay` has never existed in this record — the page fell back to the
literal "27 February 2015", right only by luck, that being the record's own
`enthronementDate` spelled out. `e.qualification` is not in the education shape either.
`m.name || m` handled a bare-string membership; every one is an object. A stale comment
claiming the portrait is null was corrected — it carries `fon-walters-ticha-ix.jpg`.

Verified behaviour-identical against production: 143 visible text blocks each, zero
differences.

### Regression and acceptance

Auth: every member API answers 401 unauthenticated, every member page 307 to sign-in.
Privacy: no personal Guneku email or key shape in any client chunk — the two gmail addresses
present are library-author attributions and `CLERK_SECRET_KEY` is a variable name inside
Clerk's vendored SDK, undefined in a browser. Held archive: 181 files, outside `public/`,
absent from the build, 404 on production. Anonymous writes: rate limit, honeypot and
server-verified Turnstile on all four. Cross-owner business access: scoped in the SQL WHERE,
so a guess returns 404 rather than confirming existence.

Search invariants were extended while the directory's first status change was fresh: every
curated business is in the index if and only if it is public, nothing of the held business is
findable — its name, tagline and description, but deliberately not its tags, which are the
shared vocabulary of the whole directory.

### Not done, and deliberately

No SEO work, no sitemap policy change, no submission to anybody. The individual Indigenes
sitemap exclusion was left exactly as it was: existing, deliberate behaviour, and the decision
about whether 113 thin entries should be submitted is the Palace's, not an engineering one.
It is written up as decision C1 in `seo-launch-handoff.md`.

### Verification

`npx tsc --noEmit` clean · `npx vitest run` **1179 passed across 47 files**, up from 1176 ·
`npm run build` succeeded at **304 static pages** · ESLint clean on every touched file ·
293 internal links crawled, 0 broken · git tree clean.
