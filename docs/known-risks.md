...
## R-047 - The navigation's active marker broke hydration on every page load - CLOSED 2026-09-07

Found by Claude in Chrome during real-user acceptance: React error **#418** — *"hydration
failed because the server rendered HTML didn't match the client"* — on every clean load of the
homepage in Production.

**The cause was the active-route marker.** It is an extra element in the tree: an underline
`<span>` in the desktop header, a dot `<div>` in the bottom bar. Both were rendered from
`usePathname()`.

**Why it passed locally and failed in Production.** `usePathname()` resolves to the route
during a local build, so `.next/server/app/index.html` carried the marker for Home.
Production's served HTML carried **zero** markers — confirmed by counting the active class in
both: 1 locally, 0 in production. These pages are regenerated by ISR
(`export const revalidate = 3600`), and in that render the hook does not resolve the same way.
The server sent a nav with nothing active; the browser, a tick later, produced a nav with the
marker; React threw the whole subtree away and re-rendered it client-side.

That is the part worth remembering: **it is not a code path that differs, it is a render
environment.** No amount of inspecting the local build output would have found it, and no test
over the build artefact would have caught it. It took loading the real page.

**Fixed** by making the first render identical by construction. `useCurrentPath()` returns
`null` during server render and the hydrating render — `useSyncExternalStore` with a server
snapshot of `false` — and the real path immediately after. The marker appears one frame late
and is correct from then on, including across client-side navigation.

Two things were deliberately not done. **`suppressHydrationWarning`** would have silenced the
error while leaving React rebuilding the header on every load; the mismatch is entirely
avoidable. And **"always render the marker, hide it with a class"** keeps the element tree
stable but moves the divergence into an attribute, which is the same bug wearing a smaller hat.

The rule is now shared and pure (`isActivePath`), so the header and the bottom bar cannot drift
apart about what "active" means — they previously each had their own comparison, and both were
wrong the same way.
...
`display_config.application_name` set to that string.

**Mitigated in the repository** by `localization` on `ClerkProvider` — the supported way to set
what the widgets say, version-controlled and reviewable. Only the strings that carried the
default are overridden; Clerk's own wording for everything else is untouched.

**Not fixed at the source, and code cannot fix it.** The application name also appears in the
verification emails Clerk sends and on the OAuth consent screen, neither of which any override
reaches.
